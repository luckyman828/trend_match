    ...
    <BaseTableInnerRow>
        <td v-if="editTitle" class="title">
            ...
            <BaseInputField
                ...
                actionOnBlur="Cancel"
                @submit="
                    insertOrUpdateTeam(teamToEdit)
                    editTitle = false
                "
                @cancel="
                    $emit('cancelEditTitle')
                    editTitle = false
                "
                @blur="
                    $emit('cancelEditTitle')
        ...
        <td class="members">
            <span>{{ team.user_count }} Members</span>
        </td>
        <!-- <td class="files">
            <v-popover :aria-disabled="team.files.length <= 0" style="display: inline-block">
                ...
                <BasePopoverList slot="popover" :header="'Team files'" :array="team.files" :arrayValueKey="'title'"/>
            </v-popover>
        </td> -->
        <td class="currency">
            <button
