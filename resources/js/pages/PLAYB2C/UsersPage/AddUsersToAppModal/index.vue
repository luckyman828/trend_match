<template>
    <BaseModal ref="addUserModal" :show="show" @close="$emit('update:show', false)" classes="sm">
        <div class="form-wrapper">
            <h3>Give users access to PLAY</h3>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('kollektApps', {
            appUsers: 'getCurrentAppUsers',
            currentApp: 'getCurrentApp',
        }),
    },
    methods: {
        ...mapActions('users', ['addUsersToWorkspace', 'insertOrUpdateWorkspaceUsers']),
        ...
        async onSubmit(e) {
            ...
            const usersToAddFiltered = this.usersToAdd.filter((user, index) => {
                ...
                const existsOnApp = !!this.appUsers.find(x => x.email.toLowerCase() == user.email.toLowerCase())
                if (existsOnApp) {
                    this.ignoredUsers.push(user)
                    return false
                }
                ...
                if (!existsOnWorkspace) usersToCreate.push(user)
                return true
            })
            this.userAddedCount = usersToAddFiltered.length
            await this.addUsersToWorkspace(usersToCreate)
            // Set the app role of the users added
            usersToAddFiltered.map(user => {
                const workspaceUser = this.users.find(x => x.email.toLowerCase() == user.email.toLowerCase())
                if (workspaceUser) {
                    workspaceUser.appRole = 'Admin'
                    Object.assign(user, workspaceUser)
                } else {
                    user.apps = [{ app: this.currentApp.name, role: 'Admin' }]
                }
            })
            await this.insertOrUpdateWorkspaceUsers(usersToAddFiltered)
