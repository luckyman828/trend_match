<template>
    <div class="conditional-filters" ref="filterWrapper">
        ...
        <div
            class="key-filter"
            v-for="(keyFilter, index) in keyFilters.filter(x => x.type == 'key')"
            :key="'key-' + index"
        >
            <v-popover trigger="click" placement="left" offset="0, -40px">
                <BaseInputField
                    class="select-key"
                    disabled="true"
                    :value="keyFilter.key.name"
                    type="select"
                    inputClass="small"
                />
                <template slot="popover">
                    <BaseSelectButtons
                        ...
                        v-model="keyFilter.key"
                    />
                </template>
            </v-popover>

            <v-popover trigger="click" placement="left" offset="0, -20px">
                <BaseInputField
                    class="select-operator"
                    disabled="true"
                    :value="keyFilter.operator"
                    type="select"
                    inputClass="small"
                />
                <template slot="popover">
        ...
        <button class="ghost" @click="onAddFilter">
        ...
        <div
            class="user-filter key-filter"
            v-for="(authorFilter, index) in keyFilters.filter(x => x.type == 'author')"
            :key="'author-' + index"
        >
            <v-popover trigger="click" placement="left" offset="0, -60px">
                <BaseInputField
                    class="select-key"
                    disabled="true"
                    :value="authorFilter.filter.name"
                    type="select"
                    inputClass="small"
                />
                <template slot="popover">
            ...
            <v-popover trigger="click">
                ...
                <template slot="popover">
                    <BaseSelectButtons
                        ...
                        optionNameKey="name"
                        optionValueKey="value"
                        :options="availableUserOperators"
                        v-model="authorFilter.operator"
                    />
                </template>
            </v-popover>
        ...
        <button class="ghost" @click="onAddAuthorFilter">
            <i class="fas fa-plus"></i>
            <span>Add user/selection filter</span>
        </button>

        <button class="primary full-width submit" @click="onSubmit" v-close-popover>
...
export default {
    ...
    methods: {
        ...
        onSubmit() {
            ...
            const filtersToSet = this.keyFilters.filter(
                x => !((x.type == 'author' && !x.filter) || (x.type == 'key' && !x.key.value))
            )
